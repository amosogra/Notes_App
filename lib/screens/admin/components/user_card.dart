            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Text.rich(
                          TextSpan(
                            ...
                            children: [
                              TextSpan(
                                text: "${widget.user!.role!.toLowerCase()} profile, ${widget.user!.email} \n",
                      ...
                      Column(
                        children: [
                          Text(
                            timeago.format((widget.user?.lastSignedTimestamp ?? widget.user?.regTimestamp ?? Timestamp.now()).toDate()),
                          ...
                          SizedBox(height: 5),
                          if (widget.user?.images?.length != 0)
                            WebsafeSvg.asset(
                  ...
                  Text(
                    "${widget.user?.firstName ?? 'Unknown'} joined this platform ${timeago.format((widget.user?.regTimestamp ?? Timestamp.now()).toDate())}, ${widget.user?.gender == 'Male' ? 'his' : widget.user?.gender == 'Female' ? 'her' : ''} last check in was ${timeago.format((widget.user?.lastSignedTimestamp ?? widget.user?.regTimestamp as Timestamp).toDate())}, Home Address is ${widget.user?.address?.address1 ?? 'not updated yet'}\n",
                    maxLines: 2,
            ...
            Positioned(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: widget.user!.isOnline! ? kBadgeOnlineColor : kBadgeOfflineColor,
                ),
              ).addNeumorphism(
                blurRadius: 4,
                borderRadius: 8,
