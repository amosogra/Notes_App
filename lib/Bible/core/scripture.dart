...
class Scripture {
  ...
  String digit(dynamic e) =>
      (e is String ? e : e.toString()).replaceAllMapped(new RegExp(r'[0-9]'), (i) => bible == null ? i.group(0)! : bible!.digit![int.parse(i.group(0)!)]);

  List<DefinitionBook>? get bookList => bible!.bookInfo;
  DefinitionBook? bookById(int? bookId) => this.bookList?.firstWhereOrNull((DefinitionBook e) => e.id == bookId);
  DefinitionBook? get bookCurrent => this.bookById(this.bookId);

  List<DefinitionTestament>? get testamentList => bible!.testamentInfo;
  DefinitionTestament? testamentById(int? testamentId) => this.testamentList!.firstWhereOrNull((DefinitionTestament e) => e.id == testamentId);
  ...
  BIBLE? get verseSearch {
    if (verseSearchDataIsEmpty()) {
      verseSearchData = new BIBLE(query: searchQuery, bookCount: 0, chapterCount: 0, verseCount: 0, info: bible!.info, book: []);
      ...
      bible!.book!.forEach((bId, bO) {
        ...
        bO['chapter'].forEach((cId, cO) {
          ...
          if (verseBlock.length > 0) {
            ...
            chapterBlock.add(new CHAPTER(id: int.parse(cId), name: this.digit(cId), verse: verseBlock));
          }
        });

        if (chapterBlock.length > 0) {
          verseSearchData!.bookCount++;
          verseSearchData!.book!.add(new BOOK(info: bookById(int.parse(bId)), chapter: chapterBlock));
        }
      });
      // log('+ verseSearch: Ok ($searchQuery)');
    }
    // log('- verseSearch: log ($searchQuery) book:${verseSearchData.bookCount} chapter:${verseSearchData.chapterCount} verse:${verseSearchData.verseCount}');
  ...
  BIBLE? get verseChapter {
    if (verseChapterDataIsEmpty()) {
      ...
      CHAPTER chapterBlock = new CHAPTER(id: this.chapterId, name: this.chapterName, verse: []);
