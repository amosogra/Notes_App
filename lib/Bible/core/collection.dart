...
abstract class _Collection with _Configuration, _Utility {
  ...
  Future<bool> deleteCollection() async => await docsExists(assetsCollection).then((String? hasExists) async {
        return (hasExists == null) ? false : await (docsDelete(assetsCollection) as FutureOr<bool>);
      });

  // Future<void> updateCollectionTest() async => Future.delayed(Duration(seconds: 3));
  Future<void> updateCollection() async => requestHTTP(_liveBookJSON.split('').reversed.join()).then((res) async {
  ...
  void switchIdentifyPrimary({bool? force}) {
    if (this.primaryId!.isEmpty || force == true) {
      this.primaryId = collection!.bible!.firstWhere((i) => i.available! > 0, orElse: () => collection!.bible!.first).identify;
    }
  }
  ...
  Future<void> addKeyword(String query) async => await collectionKeywordList().then((keyword) async {
