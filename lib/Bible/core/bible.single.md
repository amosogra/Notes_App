...
mixin _Bible on _Collection {
  ...
  Future<void> _deleteBible(String id) => docsDelete(basename(this._url(id))).then((_) async {
    ...
    // log('$id is deleted $isAvailable and currentID $identify');
  });
  ...
  BIBLE get verseSearchDispatch {
    ...
    // log('- verseSearch: log ($searchQuery) book:${verseSearchBible.bookCount} chapter:${verseSearchBible.chapterCount} verse:${verseSearchBible.verseCount}');
