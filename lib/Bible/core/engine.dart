...
Collection parseCollectionCompute(dynamic response) {
  ...
  return Collection.fromJSON(parsed)..bible!.sort((a, b) => a.order!.compareTo(b.order!));
}
...
Future<File> documents(String fileName) async => new File(join(await directory.then((e) => e.path), fileName));
...
Future<String?> docsExists(String fileName) async => await documents(fileName).then((File e) async => await e.exists() ? basename(fileName) : null);
