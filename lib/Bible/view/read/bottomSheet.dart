...
abstract class _BottomSheetState extends State<_BottomSheet> with AutomaticKeepAliveClientMixin, TickerProviderStateMixin {
  ...
  final double maxChildSize = 0.981;
...
class _BottomSheetView extends _BottomSheetState {
  ...
  Widget build(BuildContext context) {
    super.build(context);
    // log('$minChildSize $initialChildSize');
    return DraggableScrollableActuator(
      ...
      child: NotificationListener<DraggableScrollableNotification>(
        ...
        child: DraggableScrollableSheet(
            ...
            maxChildSize: /* maxChildSize */ (1 - (40 / (heightDevice + 40 * shrinkSize))),
            builder: (BuildContext _, ScrollController controller) {
              contextDraggable = _;
              scrollController = controller;
              return sheetDecoration(child: sheetShape(child: _scroll(controller)));
            }),
      ),
    );
  }
    ...
    return Scaffold(
      body: CustomScrollView(
        ...
        slivers: <Widget>[
            ...
            builder: (BuildContext context, SliverConstraints constraint) {
              return new SliverPersistentHeader(
                delegate: new ScrollBarDelegate(
                  (BuildContext context, double offset, bool overlaps, double stretch, double shrink) => _tabBarView(controller),
          ...
          //_audioBar(controller),
          //_parallelBar(controller),
        ],
      ),
    );
  }

  TabBarView _tabBarView(ScrollController controller) {
    return TabBarView(
  ...
  Widget _audioBar(ScrollController? controller) {
  ...
  Widget _parallelBar(ScrollController? controller) {
    return SliverFillRemaining(
      child: WidgetKeepAlive(
        key: keyParallel,
        child: new BottomSheetParallel(
  ...
  Widget get buttonList {
    return Row(
      ...
      children: [
        button(message: "Previous chapter", child: Icon(CustomIcon.chapter_previous, size: 25), onPressed: widget.previousChapter),
        button(message: "Next chapter", child: Icon(CustomIcon.chapter_next, size: 25), onPressed: widget.nextChapter),
        button(
        ...
        button(message: "Listen audio", child: Icon(CustomIcon.wave_square, size: 20), onPressed: showAudioIf ? showAudio : null),
        button(
...
class SliverChild extends SliverChildDelegate {
  SliverChild({this.builder});
