...
class Generator {
  ...
  static String getWords(int words, String text) {
    var listOfWords = text.split(" ");
    return listOfWords.take(words).join(" ") + (words < listOfWords.length ? "..." : "");
  }
  ...
  static Widget buildProgress({Color? activeColor, Color? inactiveColor, double progress = 1, double? height, double width = 100}) {
    ...
    return Container(
      width: width,
      height: height,
      decoration: BoxDecoration(color: inactiveColor, borderRadius: BorderRadius.all(Radius.circular(MySize.size4))),
      child: Stack(
        children: <Widget>[
          Container(
            width: width * progress,
            height: height,
            decoration: BoxDecoration(color: activeColor, borderRadius: BorderRadius.all(Radius.circular(MySize.size4))),
          )
        ],
      ),
    );
